...
export default function AdminRollcall({ room_id, rollcall, allParticipants, setRollCall }) {
    ...
    function updateQR(qrText) {
        QrCode.toDataURL(qrText, { width: 500 }, (err, url) => {
            setQrCodeUrl(url);
        })
    }
        ...
        <>
            <div className="admin-roll-cal-container">
                <div className="roll-call-action">
                    {
                        timer > 0 && <>
                            <img className="qr-image" src={qrCodeUrl} alt="QR_CODE" />
                            <h3>{timer} sec</h3>
                            <p>Time Remaining</p>
                        </>
                    }
                    <button onClick={showAbsentees} className="default-button">List Absentees</button>
                    <button title="This will just creates new rollcall removes previous" onClick={() => setRollCall(null)} className="default-button">Create And Replace</button>
                </div>
